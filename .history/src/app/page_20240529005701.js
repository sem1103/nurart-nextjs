...
export default function Home({ campaignSlides }) {
  const { setOnloadImg, imagesLoaded } = useDataSlice();
  const { t } = useTranslation();

  useEffect(() => {
    // Установка состояний компонента при загрузке
...
export async function getServerSideProps(context) {
    ...
    const { fetchCampaignSlides } = useDataSlice();
    await fetchCampaignSlides();
  
    // Возвращение данных в виде пропсов
    return {
      props: {
        campaignSlides: fetchCampaignSlides() // Пример
      }
    };
  }