...
export default function Header(){
   ...
   const router = useRouter();
    ...
    const {setSegment} =  useDataSlice();
    const {cartItems} = useDataSlice();
                    ...
                    <div className={s.right}>
                        ...
                        <div className={s.menu}>
                            ...
                            <nav>
                                <ul>
                                    <li>
                                        <Link className={segm == `prod` ?  'active' : ''} href={`/${lang}/prod/polygraphy`} onClick={() => {
                                            burgerMenuHandler();
                                        }} ref={productNav}>{t('products')}</Link>
                                    </li>
                                    <li>
                                        <Link className={segm == `promotions` ?  'active' : ''}  href={`/${lang}/promotions`} onClick={() => {
                                            burgerMenuHandler();
                                        }} >{t('promotions')}</Link>
                                    </li>
                                    <li className={s.submenu}>
                                        {t('services')}
                                        <ul>
                                            <li><Link onClick={() => burgerMenuHandler()} className={segments == `designservice` ?  'active' : ''} href={`/${lang}/designservice`}>{t('design')}</Link></li>
                                            <li><Link onClick={() => burgerMenuHandler()} className={segments == `digitprint` ?  'active' : ''} href={`/${lang}/digitprint`}>{t('digitPrint')}</Link></li>
                                            <li><Link onClick={() => burgerMenuHandler()} className={segments == `ofsetprint` ?  'active' : ''} href={`/${lang}/ofsetprint`}>{t('ofsetPrint')}</Link></li>
                                        </ul>
                                    </li>
                                    <li>
                                        <Link className={segments == `aboutUs` ?  'active' : ''} href={`/${lang}/aboutUs`} onClick={() => burgerMenuHandler()}>{t('about')}</Link>
                                    </li>
                                    <li>
                                        <Link className={segm == `contactUs` ?  'active' : ''} href={`/${lang}/contactUs`} onClick={() => burgerMenuHandler()}>{t('contact')}</Link>
                                    </li>
                                </ul>
                            </nav>
                            ...
                            <ul className={`${s.header__tools} header__tools`}>
                                ...
                                <li className={s.favorites}>
                                    ...
                                    <Link href={`/${lang}/favoritesPage`} className={segments == 'favoritesPage' ? s.active : ''} onClick={() => burgerMenuHandler()}>
                                    <svg viewBox="0 0 24 24" width="15px" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="svgRepoBgCarrier" strokeWidth="0"></g><g id="svgRepoTracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="svgRepoIconCarrier"> <path fillRule="evenodd" clipRule="evenodd" d="M12 6.00019C10.2006 3.90317 7.19377 3.2551 4.93923 5.17534C2.68468 7.09558 2.36727 10.3061 4.13778 12.5772C5.60984 14.4654 10.0648 18.4479 11.5249 19.7369C11.6882 19.8811 11.7699 19.9532 11.8652 19.9815C11.9483 20.0062 12.0393 20.0062 12.1225 19.9815C12.2178 19.9532 12.2994 19.8811 12.4628 19.7369C13.9229 18.4479 18.3778 14.4654 19.8499 12.5772C21.6204 10.3061 21.3417 7.07538 19.0484 5.17534C16.7551 3.2753 13.7994 3.90317 12 6.00019Z" stroke="#fff" strokeWidth="2" fill="transparent" strokeLinecap="round" strokeLinejoin="round"></path> </g></svg>
                                ...
                                <li className={s.card}>
                                    ...
                                    <Link href={`/${lang}/cartPage`} className={segments == 'cartPage' ? s.active : ''} onClick={() => burgerMenuHandler()}>
