...
export default function PutRequestForm({ setIsOpen }) {
    ...
    const onSubmit = async (data) => {
        try {
            setSend(1)
            await fetch("https://api.nurart.az/sendreq/request.php", {
                method: "POST",
            ...
            <div className="order__modal__wrapper" onClick={(e) => closeOrderModal(e)}>
                <div className="modal__form">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="form__group">
                            <input type="text" id="formCompanyName" {...register("formCompanyName")} className={formCompanyNameValue ? "focused" : '' } />
                            <label htmlFor="formCompanyName">{t('formCompanyName')}</label>
                        </div>
                        ...
                        <div className="form__group">
                            <button type="submit" disabled={!isValid} className={sending == 1 ? sending : ''}>{t('sendRequest')}</button>
                        </div>
                    </form>
                </div>
            </div>
