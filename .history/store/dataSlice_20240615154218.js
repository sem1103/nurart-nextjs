    ...
    devtools(
        (set, get) => ({
            ...
            fetchPromotionItems: async () => {
                try {
                    const timestamp = new Date().getTime(); // Генерируем уникальный временный штамп
                    let data = await fetch('https://api.nurart.az/wp-json/wp/v2/promotion__items/?timestamp=' + timestamp).then(res => res.json());
            ...
            fetchCampaignSlides: async () => {
                try {
                    const timestamp = new Date().getTime(); // Генерируем уникальный временный штамп
                    let data = await fetch('https://api.nurart.az/wp-json/wp/v2/campaign_slider/?timestamp=' + timestamp).then(res => res.json());
            ...
            fetchData: async (mainUrl) => {
                try {
                    ...
                    get().getCartItems();
                    const timestamp = new Date().getTime(); // Генерируем уникальный временный штамп
                    const urlWithTimestamp = `${mainUrl}?timestamp=${timestamp}`;
                    const data = await fetch(urlWithTimestamp).then(res => res.json());
                    ...
                    state => ({
                        items: {
                            ...
                            [type]: state.allItems[type].filter((item) => {
                                ...
                                const {normalizeAzerbaijani, lang} = useAzerbaijaniNormalization();
                                console.log(lang);
